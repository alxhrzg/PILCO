...
class MGPR(gpflow.Module):
    ...
    def create_models(self, data):
        ...
        for i in range(self.num_outputs):
            ...
            kern.lengthscales.prior = tfd.Gamma(
                to_default_float(1.1), to_default_float(1 / 10.0)
            )  # priors have to be included before
            kern.variance.prior = tfd.Gamma(
                to_default_float(1.5), to_default_float(1 / 2.0)
            )  # before the model gets compiled
            self.models.append(
                gpflow.models.GPR((data[0], data[1][:, i : i + 1]), kernel=kern)
            )
            self.models[-1].likelihood.prior = tfd.Gamma(
                to_default_float(1.2), to_default_float(1 / 0.05)
            )
    ...
    def calculate_factorizations(self):
        K = self.K(self.X)
        batched_eye = tf.eye(
            tf.shape(self.X)[0], batch_shape=[self.num_outputs], dtype=float_type
        )
    ...
    def predict_given_factorizations(self, m, s, iK, beta):
        ...
        t = tf.linalg.matrix_transpose(
            tf.linalg.solve(
                B,
                tf.linalg.matrix_transpose(iN),
                adjoint=True,
                name="predict_gf_t_calc",
            ),
        )
        ...
        k = tf.math.log(self.variance)[:, None] - tf.reduce_sum(tf.square(iN), -1) / 2
    ...
    def lengthscales(self):
        return tf.stack([model.kernel.lengthscales._value() for model in self.models])
        # return tf.stack([model.kernel.lengthscales for model in self.models])

    @property
    def variance(self):
        return tf.stack([model.kernel.variance._value() for model in self.models])
        # return tf.stack([model.kernel.variance for model in self.models])
