...
def squash_sin(m, s, m_width, max_action=None):
    """
    ...
    def compute_action(self, m, s, squash=True):
        ...
        if squash:
            if not hasattr(self, "m_width"):
                self.m_width = tf.shape(M)[1]

            M, S, V2 = squash_sin(
                M, S, m_width=self.m_width, max_action=self.max_action
            )
            V = V @ V2
        return M, S, V
    ...
    def create_models(self, data):
        ...
        for i in range(self.num_outputs):
            ...
            if i == 0:
                ...
                self.models.append(
                    FakeGPR((data[0], data[1][:, i : i + 1]), kernel, self.models[-1].X)
                )
